...
2. <section id="hero"> — a headline deve ser o PRIMEIRO elemento visível da seção, sem nenhum texto, tag, badge, emoji ou rótulo acima dela. ${data.headline ? `Use EXATAMENTE esta headline: "${data.headline}"` : 'Headline impactante baseada na dor+resultado'}${data.subheadline ? `. Use EXATAMENTE esta subheadline: "${data.subheadline}"` : ', subheadline abaixo'}. Botão WhatsApp CTA grande. PROIBIDO colocar qualquer texto acima da headline (sem tagline, sem nome do negócio, sem emoji decorativo, sem badge de segmento).
...
6. ${depoimentosImagens.length > 0 ? `<section id="depoimentos"> — título e subtítulo CENTRALIZADOS, carrossel de imagens de depoimentos. Use as ${depoimentosImagens.length} URLs fornecidas como <img> com object-fit:contain, max-height:320px, border-radius:12px, background:#fff. O carrossel deve ter um wrapper com position:relative e max-width:700px; margin:0 auto. As setas prev/next devem ser position:absolute; top:50%; transform:translateY(-50%) DENTRO desse wrapper (left:8px e right:8px), para ficarem coladas nas bordas da imagem — NUNCA posicione as setas fora do wrapper do carrossel. Adicione indicadores de pontos abaixo. Carrossel responsivo e touch-friendly.` : '<!-- sem depoimentos -->'}
7. <section id="cta"> — título e subtítulo CENTRALIZADOS, botão WhatsApp CTA CENTRALIZADO. Use fundo claro (cor light da paleta ou branco) com texto escuro para garantir contraste. Ícones e textos devem ser 100% visíveis — NUNCA use ícone com cor similar ao fundo
